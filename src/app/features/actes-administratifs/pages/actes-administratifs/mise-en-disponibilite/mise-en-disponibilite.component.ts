...
@Component({
  selector: 'app-mise-en-disponibilite',
  standalone: true,
  ...
  templateUrl: './mise-en-disponibilite.component.html',
  styleUrl: './mise-en-disponibilite.component.scss'
})
export class MiseEnDisponibiliteComponent implements OnInit {
  ...
  // ============================ API
  DisponibiliteData: any;
  initializeData() {
    const criteria: any = { typeActe: TypeActe.MD };
    if (this.searchDisponibilite && this.searchDisponibilite.trim().length > 0) {
      criteria.global = this.searchDisponibilite.trim();
    }
    ...
    obs.subscribe({
      next: (res) => {
        this.DisponibiliteData = res;
      },
      error: (err) => {
        console.error('Error in DisponibiliteData:', err);
  ...
  addMouvement() {
    ...
    this.newMouvement.typeActe = TypeActe.MD;
