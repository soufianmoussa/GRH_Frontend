...
export class ReintegrationComponent implements OnInit {
  ...
  // ============================ API
  reintegrationData: any;
  initializeData() {
    const criteria: any = { typeActe: TypeActe.RT };
    if (this.searchReintegration && this.searchReintegration.trim().length > 0) {
      criteria.global = this.searchReintegration.trim();
    }
    ...
    obs.subscribe({
      next: (res) => {
        this.reintegrationData = res;
      },
      error: (err) => {
        console.error('Error in reintegrationData:', err);
  ...
  addMouvement() {
    ...
    this.newMouvement.typeActe = TypeActe.RT;
