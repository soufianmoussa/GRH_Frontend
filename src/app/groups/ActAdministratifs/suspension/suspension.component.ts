...
export class SuspensionComponent implements OnInit {
  ...
  SuspensionData: any;
  initializeData() {
    const criteria: any = { typeActe: TypeActe.SP };
    if (this.searchSuspension && this.searchSuspension.trim().length > 0) {
      criteria.global = this.searchSuspension.trim();
    }
    ...
    obs.subscribe({
      next: (res) => {
        this.SuspensionData = res;
      },
      error: (err) => {
        console.error('Error in SuspensionData:', err);
  ...
  addMouvement() {
    ...
    this.newMouvement.typeActe = TypeActe.SP;
