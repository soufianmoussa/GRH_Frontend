...
export class RadiationComponent implements OnInit {
  ...
  radiationData: any;
  initializeData() {
    const criteria: any = { typeActe: TypeActe.RA };
    if (this.searchRadiation && this.searchRadiation.trim().length > 0) {
      criteria.global = this.searchRadiation.trim();
    }
    ...
    obs.subscribe({
      next: (res) => {
        this.radiationData = res;
      },
      error: (err) => {
        console.error('Error in radiationData:', err);
  ...
  addMouvement() {
    ...
    this.newMouvement.typeActe = TypeActe.RA;
