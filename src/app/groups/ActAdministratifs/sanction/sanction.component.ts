...
export class SanctionComponent implements OnInit {
  ...
  sanctionData: any;
  initializeData() {
    const criteria: any = { typeActe: TypeActe.SA };
    if (this.searchSanction && this.searchSanction.trim().length > 0) {
      criteria.global = this.searchSanction.trim();
    }
    ...
    obs.subscribe({
      ...
      error: (err) => {
        console.error('Error in sanctionData:', err);
  ...
  newMouvement: any = {};
  addMouvement() {
    this.newMouvement.typeActe = TypeActe.SA;
