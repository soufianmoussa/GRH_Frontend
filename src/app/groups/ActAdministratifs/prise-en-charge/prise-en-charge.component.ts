...
import { Tabs, TabList, TabPanel, TabPanels, Tab } from 'primeng/tabs';
...
export class PriseEnChargeComponent {
  ...
  // ============================ API
  PriseEnChargeData: any;
  initializeData() {
    const criteria: any = { typeActe: TypeActe.AD };
    if (this.searchPriseEnCharge && this.searchPriseEnCharge.trim().length > 0) {
      criteria.global = this.searchPriseEnCharge.trim();
    }
    ...
    obs.subscribe({
      next: (res) => {
        this.PriseEnChargeData = res;
      },
      error: (err) => {
        console.error('Error in PriseEnChargeData:', err);
  ...
  addMouvement() {
    ...
    this.newMouvement.typeActe = TypeActe.AD;
